...
- 優先順までロードしたい場合は、先に「Enable Priority Ordering Mode」のチェックを入れてから、プリセットをプルダウンから選択してください。
...
- 組み合わせに使用するパワーコア効果の優先順を設定します。
- ここにチェックを入れてから、④のパワーコア効果をクリックすると、優先順のリストに追加されます。(6つまで)
- このチェックボックスは、③④を操作する前にONにしないとうまく動作しません。
  
⑥ Start Monitoring / Stop Monitoring  
- パケット監視のON/OFFを制御します。
- パケット監視がONの状態でch移動を行うと、自身の所持するモジュールを読み込み、設定したパワーコアの組み合わせを探索し始めます。
  
⑦ Refilter  
- パワーコア効果の選択や優先順を変更した際に、組み合わせの再探索を指示します。
  
⑧ 指定した条件を満たすモジュールを表示するエリア  
- 設定したパワーコア効果の種類と優先順を可能な限り満たすモジュールの組み合わせが表示されます。
...
※指定したパワーコア効果のモジュールを所持していても、所持しているモジュールを組み合わせた結果、効果値の合計値が低くなってしまう場合は優先順通りにならない場合があります
![](Images/Image8.png)  
![](Images/Image9.png)
  
## 条件を変更して再探索する
1. パワーコア効果の種類と優先順を変更する
![](Images/Image10.png)
